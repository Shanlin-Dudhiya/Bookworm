...
document.addEventListener('DOMContentLoaded', function() {
    function loadBooks(category) {
        const bookListElement = document.getElementById('cat-books');
        if (bookListElement) {
            ...
            filteredBooks.forEach(book => {
                ...
                bookItem.innerHTML = `
                    ...
                    <h4><a href="bookdetail.html?id=${book.id}">${book.title}</a></h4>
                `;
    ...
    if (window.location.pathname.includes('bookdetail.html')) {
...
document.addEventListener('DOMContentLoaded', function() {
    function loadBooks(index) {
        const bookIndexElement = document.getElementById('index-books');
        if (bookIndexElement) {
            bookIndexElement.innerHTML = '';
        
            const filteredBooks = index === 'all' ? bookIndex: bookIndex.filter(book => book.index === index);
            ...
            filteredBooks.forEach(book => {
                ...
                bookItem.innerHTML = `
                    ...
                    <h4><a href="bookdetail.html?id=${book.id}">${book.title}</a></h4>
                `;
                bookIndexElement.appendChild(bookItem);
            });
        }
    }
    ...
    if (indexDropdown) {
        indexDropdown.addEventListener('change', function() {
            loadBooks(this.value);
        });
        loadBooks('all'); // Load all books initially
    }
